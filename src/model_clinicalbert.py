...
def align(record, tok):
    enc = tok(record["text"], truncation=True, max_length=MAXLEN, return_offsets_mapping=True)
...
class DS(torch.utils.data.Dataset):
    def __init__(self, records, tok):
        self.items = [align(r, tok) for r in records]
...
def predict(records, model, tok, device):
    ...
    for r in records:
        ...
        with torch.no_grad():
            tags = model(**enc).logits[0].argmax(-1).tolist()
